...
from core.models import (
    Stock, Sale, SaleItem, Order,
    FollowUpDashboard, Staff, Expense, SalaryTracker
)
...
def full_dashboard_service(period="monthly"):
    ...
    pending_amount_expr = ExpressionWrapper(
        F('total_amount') - F('advance'),
        output_field=FloatField()
    )

    pending_amount = orders_qs.aggregate(
        total_pending=Sum(pending_amount_expr)
    )["total_pending"] or 0
    ...
    orders_list = [{
        ...
        "advance": float(o.advance),
        "pending_amount": float(o.remaining_amount),
    } for o in orders_qs]
    ...
    followup_records = [{
        ...
        "status": f.status,
        "status_color": "orange" if f.status == "pending" else "green",
        "remarks": f.remarks or "",
        "is_nearest": nearest_followup and f.id == nearest_followup.id
    ...
    total_paid_salary = salary_qs.aggregate(
        total_paid=Sum("paid_amount")
    )["total_paid"] or 0

    pending_expr = ExpressionWrapper(
        F("total_salary") - F("paid_amount"),
        output_field=FloatField()
    )

    total_pending_salary = salary_qs.aggregate(
    ...
    for staff in Staff.objects.all():
        ...
        paid = trackers.aggregate(total=Sum("paid_amount"))["total"] or 0
        pending = trackers.aggregate(total=Sum(pending_expr))["total"] or 0
    ...
    def expense_total(qs, etype):
        return float(
            qs.filter(expense_type=etype)
            .aggregate(total=Sum("amount"))["total"] or 0
        )

    expense_categories = [
        {"category": "Salary", "amount": expense_total(expense_qs, "salary")},
        {"category": "Operational", "amount": expense_total(expense_qs, "operational")},
        {"category": "Others", "amount": expense_total(expense_qs, "other")},
    ]

    total_expense = sum(e["amount"] for e in expense_categories)
    ...
    return {
        ...
        "sale": {
            ...
            "monthly_amount": float(monthly_sales_amount),
            "yearly_amount": float(yearly_sales_amount),
        },
        "expense": {
        ...
        "followups": {
            "pending_count": followups_qs.count(),
